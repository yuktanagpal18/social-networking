...
function mypost()
{
	...
	var para=document.createElement("p");
	para.className="name";
	var text1=document.createTextNode("Yukta Nagpal");
	para.appendChild(text1);
	div2.appendChild(para);
	div.appendChild(div2);
	...
	var paapa=pararec.value.replace(/\s/g, "\u00a0"); //replaces all occurrences of whitespace (spaces, tabs, line breaks, etc.) with a non-breaking space character represented by "\u00a0."
	pararec.value="";
...
function decrease(likerec,dislikerec,thumbsuprec,thumbsdownrec)
{
	...
	dislike.innerHTML=parseInt(dislike.innerHTML)+1;
	var down=document.getElementById(thumbsdown);
	down.style.color= "#574753";
	down.style.pointerEvents="none";
	var up=document.getElementById(thumbsup);
	if(up.style.color=="#574753")
	{
		var like=document.getElementById(idname1);
		like.innerHTML=parseInt(like.innerHTML)-1;
	}
	up.style.color="#927a8c";
...
// load view image to be posted
 var loadFile = function(event) {
	  ...
	  //the onchange attribute in the input element calls the loadFile function when a file is selected, and the selected image will be displayed in the "load2" image element.
...
// changing imagebutton color on hover
 function onbuttoncolor()
 {
 	...
 	on.style.backgroundColor = "#f1e7e7";
 	on.style.color="#574753";
 }
...
// chaging imagebutton color on hover out
 function outbuttoncolor()
 {
 	...
 	out.style.backgroundColor = "#f7eded";
 	out.style.color="#574753";
 }
